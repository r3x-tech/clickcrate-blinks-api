export const supportedMediaFiles = ["jpg", "jpeg", "png", "gif", "svg", "mp4", "mov", "webm", "mp3", "wav", "glb", "gltf"];
...
export const validateImageUri = (imageUri: string) => {
    ...
    if (fileParts.length === 1) {
        ...
        if (!extension || !supportedMediaFiles.includes(extension)) {
            throw new Error(`Unsupported image type: ${extension ?? 'undefined'}. Supported types are: ${supportedMediaFiles.filter(ext => ['jpg', 'jpeg', 'png', 'gif', 'svg'].includes(ext)).join(', ')}`);
        }
        return extension;
    }
}