import { validateImageUri } from './serviceHelpers'; // Adjust the import path as needed
...
describe('validateImageUri', () => {
  ...
  it('should throw an error if the URL has no extension', () => {
    ...
    expect(() => validateImageUri(noExtensionUri)).toThrow('Wrong file type provided');
  });
  ...
  it('should throw an error if the file name contains only a dot', () => {
    ...
    expect(() => validateImageUri(invalidUri)).toThrow('Unsupported image type: . Supported types are: jpg, jpeg, png, gif, svg');
  });
});
